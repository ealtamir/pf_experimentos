...
Los gr\'aficos (Fig. \ref{fig1}, \ref{fig2} y \ref{fig3}) muestran que los resultados de las pruebas fueron favorables. Los valores obtenidos a partir de la simulaci\'on se corresponden con los obtenidos a partir del modelo matem\'atico. Esto es un indicador de que \textit{Bullet} debe estar usando dichos modelos para ejecutar las simulaciones.\\
Cabe destacar que los gr\'aficos que corresponden a la Fig. \ref{fig3}, presentan una discrepancia mayor entre la simulaci\'on y el modelo. Este hecho puede deberse a que dicha prueba tiene una cantidad relativamente baja de puntos, por lo tanto la resoluci\'on de la simulaci\'on no es lo suficientemente buena y el simulador tiende a cometer un mayor error.
...
\paragraph{Verificaci\'on del coeficiente de restituci\'on\\}

El segundo ensayo simula una esfera a una altura determinada sobre el suelo, que tiene una velocidad en el eje perpendicular al piso y que eventualmente colisiona contra el mismo.
...
\begin{table}[H]%
  \centering
  	\begin{tabular}{ | c || c | c | c | }
	  		\hline
	  		\textbf{Tipo de Individuo} & \textbf{Actuador} & \textbf{Funci\'on Partida} & \textbf{Fase sincronizada} \\
			\hline 
			\hline
			1 & Gen\'erico & No & S\'i\\ 
	  		\hline 
			...
			3 & Fourier & S\'i & S\'i\\ 
	  		\hline 
			4 & Extra Fourier & S\'i & S\'i\\ 
	  		\hline 
			5 & Doble coseno & S\'i & S\'i\\ 
	  		\hline 
	\end{tabular}
  \captionsetup{justification=centering}
...
\begin{table}[H]%
  \centering
  	\begin{tabular}{ | c || c | c | c | c | c |}
	  		\hline
	  		\textbf{Individuo} & Tipo 1 & Tipo 2 & Tipo 3 & Tipo 4 & Tipo 5\\
			\hline 
			\hline
			\textbf{Amplitud} &  12 & 12 & 16 & 44 & 8\\ 
	  		\hline 
			...
			\textbf{T\'ermino independiente}  & 2 & 6 & 6 & 6 & 6\\ 
	  		\hline 
			\textbf{Longitud del cromosoma} & 18 & 30 & 34 & 62 & 28\\ 
	  		\hline 
	\end{tabular}
  \captionsetup{justification=centering}
...
\end{figure}
Como puede observarse en el caso de los individuos 3, 4 y 5 (Fourier, Extra Fourier y Doble Coseno, respectivamente), se producen picos altos y pronunciados en la velocidad. Eso repercute en que la velocidad media no sea 1.3 $\frac{m}{s}$ (que es la velocidad objetivo), y ergo, en el m\'odulo de velocidad del \textit{fitness} (provocando que \'este sea m\'as bajo).\\
...
En efecto, los individuos de tipo 3 y 4 (que utilizan actuadores de Fourier y Extra Fourier, respectivamente), son los que caen de forma m\'as abrupta, y aunque intentan levantarse, vuelven a caer con la misma intensidad. \\
...
En lo que respecta al individuo de tipo 5, que utiliza como actuadores la funci\'on de doble coseno, se cae y levanta repetidamente, pero de forma m\'as suave que los que utilizan Fourier y Extra Fourier.
\begin{figure}[H]%
  \centering
