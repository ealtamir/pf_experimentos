...
\begin{figure}[H]%
   ...
   \begin{tikzpicture}[scale=0.9, every node/.style={scale=0.9}]
	...
	\draw[very thick] (8,3) -- ++(0cm,-1cm) ; %line between funcion partida and actuador in parameters
	\draw[very thick] (8,1.5) -- ++(0cm,-1cm) ; %line between funcion partida and actuador in parameters

	
	\draw (6.2,-2.85) node[font=\bf, ultra thick]{Individuo};
	\draw[thick] (6.2,-2) -- ++(0cm,-0.5cm) ;
	\draw[ultra thick] (4.9,-3.2) \rectanglepath[2.5cm][0.7cm];
	...
	\draw[thick] (9.1,-0.25) -- ++(2.2cm,0cm) ;

	\draw (-3,0.5) \rectanglepath[2.75cm][1cm];
	\draw (-1.6,1) node{Muslo izquierdo};
	...
	\draw (1.1,1.0) node{Pierna izquierda};
	...
	\draw (3.9,1.03) node{Muslo derecho};
	...
	\draw (6.65,1.03) node{Pierna derecha};
	...
	\draw (11.4,1.02) node{Pierna};
	\draw[thick] (11.3,0.5) -- ++(0cm,-0.75cm) ;
  ...
  \end{tikzpicture}
  \caption{Esquema de la composici\'on de un individuo}
  ...
  \centering
    \resizebox{\linewidth}{!}{%
  	\begin{tabular}{ | c || c | c | c | c | c |}
			...
			\hline
			 \textbf{Actuador} &  Gen\'erico & Gen\'erico & \begin{tabular}{@{}c@{}}Fourier de\\ orden 2\end{tabular} &\begin{tabular}{@{}c@{}}Fourier de\\ orden 9\end{tabular} & \begin{tabular}{@{}c@{}}Coseno doble\\ frecuencia\end{tabular}\\  
	  		\hline									    		   
...
El objetivo principal de este proyecto fue lograr producir la simulaci\'on biomec\'anica de una caminata de un humano virtual. Para eso, se eligi\'o el motor f\'isico \textit{Bullet Physics}, lo que requiri\'o no solo aprender sobre su funcionamiento y los m\'etodos f\'isicos implementados, sino tambi\'en realizar pruebas para verificar qu\'e tan pr\'oximos eran el modelo fisico-matem\'atico ideado y el utilizado por Bullet.\\ % , en temas de coeficiente de fricci\'on y restituci\'on.\\
