...
\usepackage[labelformat=simple]{subcaption} % Required to add parentheses to subfigures references
...
\author{  \small{\textbf{Instituto Tecnol\'ogico de Buenos Aires - ITBA \\  INGENIER\'IA INFORM\'ATICA} }  }
\affil{\vspace*{0ex}}
\begin{document}
...
En base a los modelos f\'isico-matem\'aticos utilizados  en los dos fen\'omenos en cuesti\'on (y que se explicar\'an a continuaci\'on), se llevaron a cabo dos experimentos para verificar que estuvieran en concordancia con los datos arrojados por \textit{Bullet}:\\
\begin{itemize}
...
\end{equation}
\\ En este caso, el cuerpo empieza su movimiento en el origen, por lo tanto la posici\'on inicial ($x_i$) es cero.\\
Debido a la fricci\'on entre el cuerpo y el suelo, se genera una fuerza de rozamiento $F_{\mu_{d}}$ en ec. (2) en la misma direcci\'on que la velocidad del s\'olido y en sentido contrario. \\
\begin{equation}
  -F_{\mu_d} = \mu_d F_N 
\end{equation}
...
\begin{figure}[H] 
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=1.0\linewidth]{image002.png} 
    \caption{ } 
    \label{fig1:a} 
    \vspace{4ex}
  \end{subfigure}%% 
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=1.00\linewidth]{image003.png} 
    \caption{ } 
    \label{fig1:b} 
    \vspace{4ex}
  \end{subfigure} 
...
\begin{figure}[H] 
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=1.0\linewidth]{image009.png} 
    \caption{ } 
    \label{fig2:a} 
    \vspace{4ex}
  \end{subfigure}%% 
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=1.0\linewidth]{image007.png} 
    \caption{ } 
    \label{fig2:b} 
    \vspace{4ex}
  \end{subfigure} 
...
\begin{figure}[H] 
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=1.0\linewidth]{image004.png} 
    \caption{ } 
    \label{fig3:a} 
    \vspace{4ex}
  \end{subfigure}%% 
  \begin{subfigure}[b]{0.5\linewidth}
    \centering
    \includegraphics[width=1.0\linewidth]{image005.png} 
    \caption{ } 
    \label{fig3:b} 
    \vspace{4ex}
  \end{subfigure} 
  \begin{subfigure}[b]{\linewidth}
    \centering
    \includegraphics[width=0.5\linewidth]{image006.png} 
    \caption{ } 
    \label{fig3:c} 
  \end{subfigure}%%
...
\item El segundo simula una esfera a una altura determinada sobre el suelo, que tiene una velocidad en el eje perpendicular al piso y que eventualmente colisiona contra el mismo. Se desea comprobar que la colisi\'on entre el cuerpo y el suelo respete que la velocidad final de la esfera despu\'es del choque sea proporcional a su coeficiente de restituci\'on $e$ dado por la ecuaci\'on:\\ 
 \begin{equation}
 \label{eq:restitution_coef}
  e = \frac{v_f}{v_i} 
\end{equation}
\\ \\
Las siguientes tablas muestran los experimentos realizados. En ellos, se mantienen fijos $v_{i}$ (velocidad inicial) y $e_{sim}$ (coeficiente de restituci\'on esperado); se obtiene $v_{f}$ (velocidad de la esfera al finalizar la simulaci\'on); y se calculan $e_{medida}$ (coeficiente de restituci\'on obtenido a partir de \eqref{eq:restitution_coef}) y $\epsilon_{rel}$ (error relativo entre los coeficientes): \\

\begin {table}[H]
	\begin{center}
		\begin{tabular}{ | c || c | c | c | }
	  		 \hline
	  		$v_{i}$ & \multicolumn{3}{c |}{-0.5}\\
	  		\hline 
	  		$v_{f}$ & 0.000249 & 0.000219 & 0.001037\\
	  		\hline
	  		...
	  		\hline
		\end{tabular}
		\caption{Coeficiente de fricci\'on de 0.75 }
		\label{table:Exp1}
	\end{center}
\end{table}
...
\begin {table}[H]
	\begin{center}
		\begin{tabular}{ | c || c | c | c | }
	  		 \hline
	  		$v_{i}$ & \multicolumn{3}{c |}{-3.5}\\
	  		\hline 
	  		$v_{f}$ & 0.000057 & 0.000018 & 0.3\\
	  		\hline
...
\begin {table}[H]
	\begin{center}
		\begin{tabular}{ | c || c | c | c | }
	  		 \hline
	  		$v_{i}$ & \multicolumn{3}{c |}{-4}\\
	  		\hline 
	  		$v_{f}$ & 0.000473 & 0.000424 & 1.23\\
	  		\hline
	  		...
	  		\hline
		\end{tabular}
		\caption{Con coeficiente de fricci\'on de 0.75 }
		\label{table:Exp3}
	\end{center}
\end{table}
...
Los resultados exponen una limitaci\'on del motor f\'isico: no representa correctamente las colisiones el\'asticas entre esferas y cuerpos r\'igidos, que ocurren a velocidades bajas. Esto queda en evidencia en las Tablas \ref{table:Exp1}, \ref{table:Exp2}, \ref{table:Exp3} y \ref{table:Exp4}. En cada una de ellas el error fue de casi el 100\%.\\
La raz\'on por la que ocurre este hecho se debe a que \textit{Bullet} utiliza un algoritmo de colisi\'on que frena la velocidad de un objeto que est\'a a punto de colisionar. Haciendo esto puede evitar que los s\'olidos se traspasen y de esta forma se pueden realizar c\'alculos de fuerza m\'as precisos. En el caso de los experimentos, las esferas poseen una rapidez muy baja, cuando est\'an a punto de colisionar \textit{Bullet} reduce a\'un m\'as esta velocidad y eventualmente quedan con una velocidad tan baja que al chocar contra el suelo se aplica el efecto restitutivo a esta rapidez casi nula y se resuelve que la esfera debe quedar en reposo, cuando en realidad deber\'ia poseer una velocidad baja, pero no despreciable. \\
...
El programa incluye tanto la visualizaci\'on gr\'afica de la simulaci\'on f\'isica de \textit{Bullet} del humanoide, as\'i como el algoritmo gen\'etico -y la definic\'on de los individuos, fitness, etc. y su integraci\'on con GaLib-.
...
Es el actuador m\'as sencillo (tanto matem\'atica como computacionalmente). No se consideraron las funciones trigonom\'etricas b\'asicas (es decir, solo $\sin$ o $\cos$), por los resultados obtenidos en el trabajo del Cuadr\'upedo, antes mencionado. La fase es la misma tanto en el seno como en el coseno, para evitar que se formen otro tipo de funciones no cíclicas ($f(t)=0$).
\begin{equation}
...
Su funci\'on est\'a dada por dividir a la función periódica en dos partes, cada una de ellas con funciones cosenoidales de frecuencias distintas.  
\begin{equation}
\begin{aligned}[c]
...
\begin{equation}
  \text{fitness} = \text{altura} * \text{velocidad} * \text{direcci\'on} * \text{simetr\'ia} *  \text{pies abajo}
\end{equation}
...
\begin{equation}
  \text{altura} = \frac{\sum_{n=0}^{T} {e^{-C( h_{t_{n}} - h_{t_{0}} )^2  }}}{\textit{simulation steps}}
\end{equation}
...
\begin{equation}
  \text{velocidad} = \frac{\sum_{n=0}^{T} {e^{-C( \|v_{t_{n}}\| - V_{O} )^2  }}}{\textit{simulation steps}}
\end{equation}
...
\begin{equation}
 \text{direcci\'on} = \frac{\sum_{n=0}^{T} {e^{-C( \vec{v_{t_{n}}} \cdot \vec{V_{O} } -1)^2 } } }{\textit{simulation steps}}
\end{equation}
...
Para mayor simplicidad, la simetr\'ia se calcul\'o a partir de los pies (y no de las piernas). Se tomaron en cuenta s\'olo los ejes X y Z, porque son los relacionados a la velocidad y a la direcci'on, respectivamente.
\begin{equation}
 \text{simetr\'ia} = \frac{\sum_{n=0}^{T} { \frac{1}{2} [e^{-C( lfoot_{Z} + rfoot_{Z}| ^2) } + e^{-C( lfoot_{X} + rfoot_{X}| ^2) }]  } } {\textit{simulation steps}}
\end{equation}
...
\begin{equation}
 \text{pies abajo} = \frac{\sum_{n=0}^{T} { \frac{1}{2} ( \alpha  [e^{-C( l\_diff\_foot ^2) } + e^{-C( r\_diff\_foot ^2) }]  } } {\textit{simulation steps}}
\end{equation}
