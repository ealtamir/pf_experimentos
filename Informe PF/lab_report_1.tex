...
Debido a la fricci\'on entre el cuerpo y el suelo, se genera una fuerza de rozamiento $F_{\mu_{d}}$ -ver ec. (2)- en la misma direcci\'on que la velocidad del s\'olido y en sentido contrario. \\
\begin{equation}
  -F_{\mu_d} = \mu_d F_N 
\end{equation}
...
%\begin{adjustwidth}{1.25cm}{0cm}
El papel de la funci\'on de fitness en un algoritmo gen\'etico es evaluar qu\'e tan bueno es un individuo. En este caso, est\'a definida como un producto de cinco m\'odulos o propiedades: altura, velocidad, direcci\'on, simetr\'ia y pies abajo:
\begin{equation}
  \text{fitness} = H * V *D * S *  PA
\end{equation}
...
\begin{equation}
  H = \frac{\sum_{n=0}^{T} {e^{-C( h_{t_{n}} - h_{t_{0}} )^2  }}}{\textit{simulation steps}}
\end{equation}
\\ donde $t_{0}$ es el tiempo inicial, $t_{T}$ el tiempo final, simulation\_steps la cantidad pasos de simulaci\'on y C una constante $C=5$.
\\
...
\begin{equation}
  V = \frac{\sum_{n=0}^{T} {e^{-C( \|v_{t_{n}}\| - V_{O} )^2  }}}{\textit{simulation steps}}
\end{equation}
...
\begin{equation}
 D = \frac{\sum_{n=0}^{T} {e^{-C( \vec{v_{t_{n}}} \cdot \vec{V_{O} } -1)^2 } } }{\textit{simulation steps}}
\end{equation}
...
\begin{equation}
 S = \frac{\sum_{n=0}^{T} { \frac{1}{2} [e^{-C( lfoot_{Z} + rfoot_{Z}| ^2) } + e^{-C( lfoot_{X} + rfoot_{X}| ^2) }]  } } {\textit{simulation steps}}
\end{equation}
...
\begin{equation}
 PA = \frac{\sum_{n=0}^{T} { \frac{1}{2} ( \alpha  [e^{-C( l\_diff\_foot ^2) } + e^{-C( r\_diff\_foot ^2) }]  } } {\textit{simulation steps}}
\end{equation}
